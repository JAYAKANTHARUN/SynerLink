...
const Find = () => {
  ...
  useEffect(() => {
    ...
    if (useremail) {
      findprojects(useremail);
    }  
  }, []);
  
  const findprojects = async (email) => {
    let result = await fetch("http://192.168.29.250:5000/find_projects", {
      method: "post",
          ...
          <Card key={index} className="w-full max-w-[48rem] flex-row bg-[#F3E1E1] rounded-tl-[80px] rounded-r-none rounded-b-none ">
            ...
            <CardBody>
              ...
              <a href="#" className="inline-block">
                <Button onClick={(e) => goToProjectPage(project.project_name,e)} variant="text" className="flex items-center gap-2 text-[#503C3C] ">
                  Learn More
                  <svg
